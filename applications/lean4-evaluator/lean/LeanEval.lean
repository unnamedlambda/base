...
def stringToBytes (s : String) : List UInt8 :=
  s.toUTF8.toList ++ [C.ZERO_U8]

def padTo (bytes : List UInt8) (targetLen : Nat) : List UInt8 :=
  bytes ++ List.replicate (targetLen - bytes.length) C.ZERO_U8

def zeros (n : Nat) : List UInt8 :=
  List.replicate n C.ZERO_U8

def uint32ToBytes (n : UInt32) : List UInt8 :=
  let b0 := UInt8.ofNat (n.toNat &&& C.BYTE_MASK_N)
  let b1 := UInt8.ofNat ((n.toNat >>> C.SHIFT_8_N) &&& C.BYTE_MASK_N)
  let b2 := UInt8.ofNat ((n.toNat >>> C.SHIFT_16_N) &&& C.BYTE_MASK_N)
  let b3 := UInt8.ofNat ((n.toNat >>> C.SHIFT_24_N) &&& C.BYTE_MASK_N)
  [b0, b1, b2, b3]
...
def MAIN_FLOW_LEN : Nat :=
  SETUP_LEN + SKIP_PARSE_LEN + ADD_TO_RESULT_LEN + CHECK_PLUS_LEN +
