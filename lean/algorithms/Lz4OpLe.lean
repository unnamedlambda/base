...
theorem cursorAtSites_shipped (inPtr outPtr : Nat) (gm : Array UInt8) (smemB : List UInt8)
    ...
    CursorAtSites 15 inPtr outPtr gm smemB := by
  ...
  have hsites : (siter K (k + 1) (initSt w.val inPtr outPtr gm smemB)).pc = 130
      ∨ (siter K (k + 1) (initSt w.val inPtr outPtr gm smemB)).pc = 140
      ∨ (siter K (k + 1) (initSt w.val inPtr outPtr gm smemB)).pc = 147
      ∨ (siter K (k + 1) (initSt w.val inPtr outPtr gm smemB)).pc = 173
      ∨ (siter K (k + 1) (initSt w.val inPtr outPtr gm smemB)).pc = 178
      ∨ (siter K (k + 1) (initSt w.val inPtr outPtr gm smemB)).pc = 188
      ∨ (siter K (k + 1) (initSt w.val inPtr outPtr gm smemB)).pc = 195
      ∨ (siter K (k + 1) (initSt w.val inPtr outPtr gm smemB)).pc = 216
      ∨ (siter K (k + 1) (initSt w.val inPtr outPtr gm smemB)).pc = 226
      ∨ (siter K (k + 1) (initSt w.val inPtr outPtr gm smemB)).pc = 233 := by
    simpa only [sbAddrSites, List.mem_cons, List.not_mem_nil, or_false] using hsite
  -- the seven token stores
  have HTOK : ((siter K (k + 1) (initSt w.val inPtr outPtr gm smemB)).pc = 130
      ∨ (siter K (k + 1) (initSt w.val inPtr outPtr gm smemB)).pc = 140
      ∨ (siter K (k + 1) (initSt w.val inPtr outPtr gm smemB)).pc = 147
      ∨ (siter K (k + 1) (initSt w.val inPtr outPtr gm smemB)).pc = 173
      ∨ (siter K (k + 1) (initSt w.val inPtr outPtr gm smemB)).pc = 178
      ∨ (siter K (k + 1) (initSt w.val inPtr outPtr gm smemB)).pc = 188
      ∨ (siter K (k + 1) (initSt w.val inPtr outPtr gm smemB)).pc = 195) →
      ((siter K (k + 1) (initSt w.val inPtr outPtr gm smemB)).regs "op" l).toNat
        ≤ (WP.mk 15).lenOff := by
    intro hq
    have hmemB : (siter K (k + 1) (initSt w.val inPtr outPtr gm smemB)).pc ∈ bodyRegion := by
      rcases hq with e | e | e | e | e | e | e <;> rw [e] <;> decide
    obtain ⟨hge, -, -⟩ :=
      body_entry_at (initSt w.val inPtr outPtr gm smemB) preSteps hpc39 (k + 1) hmemB
    have hsplit : siter K (k + 1 - (preSteps + 1))
        (siter K (preSteps + 1) (initSt w.val inPtr outPtr gm smemB))
        = siter K (k + 1) (initSt w.val inPtr outPtr gm smemB) := by
      rw [← siter_add K (preSteps + 1) (k + 1 - (preSteps + 1)),
        show preSteps + 1 + (k + 1 - (preSteps + 1)) = k + 1 from by omega]
    have hfin := tok_sites_bounded (WP.mk 15).lenOff (by decide)
      (siter K (preSteps + 1) (initSt w.val inPtr outPtr gm smemB)) (by rw [hpc40]; decide)
      l (k + 1 - (preSteps + 1))
      (fun j _ hj => by
        rw [← siter_add K (preSteps + 1) j] at hj ⊢
        exact hck j hj)
      (by rw [hsplit]; exact hq)
    rw [hsplit] at hfin
    exact hfin
  -- the three stores of the final literal run
  have HTAIL : ((siter K (k + 1) (initSt w.val inPtr outPtr gm smemB)).pc = 216
      ∨ (siter K (k + 1) (initSt w.val inPtr outPtr gm smemB)).pc = 226
      ∨ (siter K (k + 1) (initSt w.val inPtr outPtr gm smemB)).pc = 233) →
      ((siter K (k + 1) (initSt w.val inPtr outPtr gm smemB)).regs "op" l).toNat
        ≤ (WP.mk 15).lenOff := by
    intro hq
    have hkn : preSteps + 1 + n ≤ k + 1 := by
      ...
      · exfalso
        have h210 : 210 ≤ (siter K (k + 1) (initSt w.val inPtr outPtr gm smemB)).pc := by
          rcases hq with e | e | e <;> rw [e] <;> omega
        have hst := stays_from_210 (initSt w.val inPtr outPtr gm smemB) (k + 1) h210
          (preSteps + 1 + n) (by omega)
        rw [hpc209] at hst; omega
      · exact hge
    have hsplit : siter K (k + 1 - (preSteps + 1 + n))
        (siter K (preSteps + 1 + n) (initSt w.val inPtr outPtr gm smemB))
        = siter K (k + 1) (initSt w.val inPtr outPtr gm smemB) := by
      rw [← siter_add K (preSteps + 1 + n) (k + 1 - (preSteps + 1 + n)),
    ...
    have hfin := tail_sites_bounded (WP.mk 15).lenOff (by decide)
