...
/-
  Persistent CUDA VecAdd: x/y stay on GPU between prep and infer.

  One Base instance, three Algorithm objects:
    u0:1  load   — init CUDA, allocate metadata/x/y buffers, upload n
    u0:2  prep   — upload x and y to persistent buffers
    u0:3  infer  — launch PTX kernel, optional download from y buffer

  Data formats:
    load  data: [n: u64]
    prep  data: [x: n f32][y: n f32]
    infer out:  [x + y: n f32]  (optional; compute-only if out_len=0)
-/
...
def main : IO Unit := do
  let json := Json.arr #[
    toJson CudaVecAddPersist.buildConfig,
    toJson CudaVecAddPersist.loadAlgorithm,
    toJson CudaVecAddPersist.prepAlgorithm,
    toJson CudaVecAddPersist.inferAlgorithm
  ]
