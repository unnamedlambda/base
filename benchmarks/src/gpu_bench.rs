...
fn burn_vec_add_gpu(a: &[f32], b: &[f32], device: &burn::backend::wgpu::WgpuDevice) -> f64 {
    ...
    result.as_slice::<f32>().unwrap().iter().map(|&x| x as f64).sum()
}

fn burn_matmul_gpu(a: &[f32], b: &[f32], n: usize, device: &burn::backend::wgpu::WgpuDevice) -> f64 {
    use burn::tensor::{Tensor, TensorData};
...
fn burn_reduction_gpu(data: &[f32], num_groups: usize, device: &burn::backend::wgpu::WgpuDevice) -> f64 {
    use burn::tensor::{Tensor, TensorData};
    let n = data.len();
