...
// No AsyncDispatch/Wait overhead per network op.
// ---------------------------------------------------------------------------
...
fn build_clif_net_algorithm(port: u16, data_size: usize) -> base::Algorithm {
    ...
    base::Algorithm {
        ...
        stack_size: Some(256 * 1024),
        timeout_ms: Some(120_000),
        thread_name_prefix: Some("clif-net-bench".into()),
    }
}
...
pub fn run(iterations: usize) -> Vec<NetworkBenchResult> {
    ...
    eprintln!("  CLIF+Net: Cranelift JIT calls extern C network primitives\n");
    ...
    for (bench_idx, &kb) in [1, 10, 100].iter().enumerate() {
        ...
        for _iter in 0..iterations {
            ...
            let clif_alg = build_clif_net_algorithm(port, data_size);
            let test_data_clone = test_data.clone();

            thread::spawn(move || {
                let _ = base::execute(clif_alg);
            });

            let start = std::time::Instant::now();
            let _response = tcp_client(port, &test_data_clone);
            let elapsed = start.elapsed().as_secs_f64() * 1000.0;
            clif_times.push(elapsed);
            thread::sleep(Duration::from_millis(10));
        }

        let mut sorted = clif_times.clone();
        sorted.sort_by(|a, b| a.partial_cmp(b).unwrap());
        let clif_ms = sorted[sorted.len() / 2];
        ...
        if !verify_clif {
            eprintln!("  VERIFY FAIL: CLIF+Net echo incorrect");
        }
        ...
        results.push(NetworkBenchResult {
            ...
            base_ms,
            clif_ms,
            verified: verify_raw && verify_base && verify_clif,
        });
    }
